...
class WorkerManager:
    ...
    """
    def __init__(self, workers: dict, logger, log_dir: str, queue_host: str, queue_port: int, db_uri: str):
    ...
    def start_worker(self, worker_name: str, instance_id: int = 0) -> Optional[subprocess.Popen]:
        ...
        with open(log_file_path, 'a') as stdout_target:
            self.logger.info(f"Starting {worker_name} (instance {instance_id}): {' '.join(cmd)}")
            ...
            self.logger.info(f"Started {worker_name} (instance {instance_id}) with PID {process.pid}")
        self.update_worker_status(worker_name, 'running')
        return process

    def update_worker_status(self, worker_name: str, status: str, message: str = None) -> None:
    ...
    def start_all_workers(self) -> bool:
        ...
        for worker_name, config in self.workers.items():
            if config['required'] and (
                worker_name not in self.worker_processes or
                not self.worker_processes[worker_name]
            ):
    ...
    def shutdown_worker(self, worker_name: str, process: subprocess.Popen, timeout: int = 30) -> bool:
        """Shutdown a worker process."""
        try:
            if process.poll() is not None:
                self.logger.info(f"Worker {worker_name} (PID {process.pid}) already terminated")
                return True
            ...
            if process.poll() is None:
                ...
                return False
            self.logger.info(f"Worker {worker_name} (PID {process.pid}) terminated gracefully")
            return True
        except Exception as e:
            self.logger.error(f"Error shutting down worker {worker_name} (PID {process.pid}): {str(e)}")
            return False
